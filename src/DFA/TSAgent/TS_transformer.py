...
class TSFunctionTransformer:
                    ...
                    if sub_node.type == "argument_list":
                        for child in sub_node.children:
                            ...
                            if child.type == "call_expression":
                                ...
                                sub_mi_nodes = TSParser.find_nodes(
    ...
    def remove_comments_in_function(self):
        ...
        nodes = TSParser.find_nodes(root_node, "comment")
        ...
        nodes = TSParser.find_nodes(root_node, "preproc_ifdef")
        self.SSI_without_comments = self.SSI
        for node in nodes:
            ifdef_code = self.SSI[node.start_byte : node.end_byte]
            self.SSI_without_comments = self.SSI_without_comments.replace(
                ifdef_code, "\n" * ifdef_code.count("\n")
            )
        return
