...
class BatchRun:
    ...
    def start_batch_run_llm_hal_spot(self, main_test_file, main_test_track: str, project_mode: str,
                                     neural_check_strategy: Dict[str, bool], is_measure_token_cost: bool = False) -> None:
        ...
        total_cnt = 0
        inference_log_dir_path = str(Path(__file__).resolve().parent / "../../log/initial_inference/")
        if not os.path.exists(inference_log_dir_path):
            os.makedirs(inference_log_dir_path)
        inference_log_dir_path = str(
            Path(__file__).resolve().parent / "../../log/initial_inference" / self.inference_model_name
        )
        if not os.path.exists(inference_log_dir_path):
            os.makedirs(inference_log_dir_path)
    ...
    def start_batch_baselines(self, main_test_file, main_test_track, project_mode: str,
...
def run_dev_mode():
    bug_types = ["juliet-test-suite-DBZ", "juliet-test-suite-NPD", "juliet-test-suite-XSS", "juliet-test-suite-CI",
    ...
    parser.add_argument("--bug-type", choices=["xss", "dbz", "npd", "ci", "apt"],
                        help="Specify the bug type")
    parser.add_argument("--inference-model", choices=["gpt-3.5-turbo-0125", "gpt-4-turbo-preview",
                                                      "claude-3-opus-20240229", "gemini", "claude-3-haiku-20240307",
                                                      "claude-3-sonnet-20240229"],
                        help="Specify LLM model for Inference")
    parser.add_argument("--validation-model", choices=["gpt-3.5-turbo-0125", "gpt-4-turbo-preview",
                                                       "claude-3-opus-20240229", "gemini", "claude-3-haiku-20240307",
                                                       "claude-3-sonnet-20240229"],
                        help="Specify LLM model for Validation")

    parser.add_argument("--analysis-mode", choices=["lazy", "eager"],
                        help="Specify analysis mode: lazy (load original reports) or eager (re-analyze)")
    parser.add_argument("--project-mode", choices=["single", "partial", "all"],
                        help="Specify the project mode: a single file, a single track, and all files")
    parser.add_argument("--engine", choices=["llmhalspot", "baseline"],
                        help="Specify the analyzer: llmhalspot or baseline")

    parser.add_argument("-function-check", action="store_true", help="Enable function check")
    parser.add_argument("-escape-check", action="store_true", help="Enable escape check")
    parser.add_argument("-intra-dataflow-check", action="store_true", help="Enable intra dataflow check")

    parser.add_argument("-measure-token-cost", action="store_true", help="Measure token cost")
    parser.add_argument("--global-temperature",
                        choices=["0.0", "0.5", "0.7", "1.0", "1.5", "1.7", "2.0"],
                        help="Specify the temperature")

    parser.add_argument("-step-by-step-verification", action="store_true", help="Enable the step-by-step verification")
    parser.add_argument("--self-consistency-k",
    ...
    batch_run = BatchRun(spec, project_name, inference_model, inference_model_key,
                         validation_model, validation_model_key, analysis_mode)

    # LLMHalSpot should be run before Baselines
    if args.engine == "llmhalspot":
        batch_run.start_batch_run_llm_hal_spot(main_test_file, main_test_track, project_mode,
                                               neural_check_strategy, is_measure_token_cost)
    elif args.engine == "baseline":
        batch_run.start_batch_baselines(main_test_file, main_test_track, project_mode,
                                        is_measure_token_cost,
                                        step_by_step_verification,
                                        global_self_consistency_k,
...
def run_release_mode():
    ...
    for main_test_file in test_cases:
        ...
        batch_run = BatchRun(spec, project_name, inference_model, inference_model_key, validation_model,
                             validation_model_key, analysis_mode)
